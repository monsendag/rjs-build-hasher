...
module.exports = function (output, options) {
  ...
  product.bundles.forEach(function (bundleArr) {
    ...
    var b = bundles[fileName] = {
      fileName: fileName,
      baseName: baseName,
      ext: ext,
      sourcePath: path.join(options.sourcePath, fileName),
    ...
    b.newPath = path.join(path.dirname(b.filePath), b.newName);

    // allow defining a documentRoot to get link paths relative to it
    if('documentRoot' in options) {
      b.publicSourcePath = path.relative(options.documentRoot, b.sourcePath);
