...
module.exports = function (output, options) {
  ...
  product.bundles.forEach(function (b) {
    // filename
    var fileName = b.parent;
    // extension
    b.ext = path.extname(fileName);
    // basename
    b.baseName = path.basename(fileName, b.ext);
    // path to file
    b.filePath = path.join(options.buildPath, fileName);
